...
export async function GET(r:Request){const x=no(r);if(x)return x;let q=await db.query("SELECT * FROM organization_settings WHERE id='default'");if(q.rows.length===0){q=await db.query("INSERT INTO organization_settings(id,name,legal_name,monthly_goal) VALUES('default','Custom Graphics','Custom Graphics',600000) RETURNING *")}return NextResponse.json(map(q.rows[0]))}
